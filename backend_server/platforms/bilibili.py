        ...
        try:
            ...
            async with httpx.AsyncClient(headers=self._h,timeout=15)as c:
                ...
                for qn in[116,112,80,74,64]:
                    r2=await c.get(f"https://api.bilibili.com/x/player/playurl?bvid={vid}&qn={qn}",headers={**self._h,"Referer":"https://www.bilibili.com/video/"+vid})
                    d2=r2.json()
                    if d2.get("code")==0and d2.get("data",{}).get("durl"):vu=d2["data"]["durl"][0].get("url","")or d2["data"]["durl"][0].get("backup_url",[""])[0];break
                if not vu:
                    try:return await self._parse_with_ytdlp(url)
                    except:vu=""
                if not vu:
                    r2=await c.get(f"https://api.bilibili.com/x/player/wbi/v2?bvid={vid}&qn=80",headers={**self._h,"Referer":"https://www.bilibili.com/video/"+vid})
                    d2=r2.json()
                    if d2.get("code")==0and d2.get("data",{}).get("durl"):vu=d2["data"]["durl"][0].get("url","")or d2["data"]["durl"][0].get("backup_url",[""])[0]
