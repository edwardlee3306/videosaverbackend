...
class BaseParser(ABC):
    ...
    async def _parse_with_ytdlp(self,url):
        try:
            r=subprocess.run(["yt-dlp","--dump-json","--no-warnings","--format","best[ext=mp4]/best","--user-agent",self._headers["User-Agent"],url],capture_output=True,text=True,timeout=30)
            if r.returncode!=0:raise RuntimeError(r.stderr[:200])
            d=json.loads(r.stdout.strip().split('\n')[0])
            vu=d.get("url","")or self._best_url(d)
            return {"title":d.get("title",""),"video_url":vu,"cover_url":d.get("thumbnail","")or(d.get("thumbnails",[{}])[0].get("url","")if d.get("thumbnails")else""),"duration":d.get("duration",0),"file_size":d.get("filesize",d.get("filesize_approx",0)),"width":d.get("width",0),"height":d.get("height",0),"resolution":d.get("resolution",""),"fps":d.get("fps",30),"ext":"mp4","extractor":"yt-dlp","watermark":"unknown"}
